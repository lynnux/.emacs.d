    ...
    `(progn
       ...
       (defun ,name ()
         ...
         (let ((repeat-message-function #'ignore))
           ...
           (repeat nil)))
       ',name)))
(use-package s :commands (s-split s-word-wrap))
...
(use-package
 ...
 (use-package
  ...
  :init (global-set-key (kbd "<C-f1>") 'dired-sidebar-toggle-sidebar)
  (setq
 ...
 (defun delay-dired-relate-init ()
  ...
  "
   ...
   (use-package
    dired-ranger
    :config (define-key dired-mode-map "z" 'dired-do-compress-to)
...
(use-package
 ...
 :init
 (global-set-key
  (kbd "C-a") 'mwim-beginning-of-code-or-line)
...
(use-package
 ...
 (defvar tempel-key-syntaxes
   (list #'tempel-try-key-from-whitespace "w_.()" "w_." "w_" "w"))
 (defun tempel--templates-for-key-at-point ()
   ...
   (save-excursion
     (let ((original (point))
           ...
           (templates)
           (method))
       (while (and methods (not templates))
         ...
         (cond
          ...
          (t
           (setq methods (cdr methods))
           (yas--warning
            "Invalid element `%s' in `tempel-key-syntaxes'" method)))
         (let ((possible-key
                (buffer-substring-no-properties (point) original)))
           (save-excursion
             (goto-char original)
 ...
 (defun my-tempel-expandable-p ()
   ...
   (when (and
     ...
     (let ((s
            (car-safe (tempel--templates-for-key-at-point))
...
(use-package
 yasnippet
 :defer t
 :config (setq yas-global-mode t) (defvar yas-minor-mode t)
...
(use-package google-c-style :commands (google-set-c-style))
...
(use-package
 avy-zap
 :bind
 (("M-z" . avy-zap-to-char-dwim) ("M-Z" . avy-zap-up-to-char-dwim)))
...
(use-package
 ...
 :init (defun ivy-set-occur (a b)) (defun ivy-configure (a b c))
 :config
...
(progn
  ...
  (use-package
   ...
   (use-package vertico-mouse :config (vertico-mouse-mode)))
...
")
 (use-package citre-util :commands (citre-tags-file-path))
 (use-package
  etags
  :defer t
  :commands
    ...
    (let ((cmdproxy-old-encoding
           (cdr
            (assoc
             ...
             process-coding-system-alist))))
      (modify-coding-system-alist
       'process "[cC][mM][dD][pP][rR][oO][xX][yY]" '(utf-8 . gbk-dos))
      ad-do-it
      (modify-coding-system-alist
       'process
       "[cC][mM][dD][pP][rR][oO][xX][yY]"
...
(use-package
 ...
 :init (autoload 'flycheck-mode "lsp/flycheck")
...
(use-package
 ...
 (use-package
  ...
  :init (global-set-key (kbd "C-c h") #'diff-hl-show-hunk)
...
(use-package
 ...
 :defer t
 :init (defalias 'note-preview 'maple-preview-mode)
 (autoload
...
(use-package
 ...
 :init (setq org-roam-db-gc-threshold most-positive-fixnum)
   ...
   (setq org-roam-capture-templates
         '(("d" "default" plain "%?"
           ...
           ("t" "tech" plain "%?"
            :target
            (file+head
             "tech/%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
            :unnarrowed t)
           ("w" "work" plain "%?"
            :target
            (file+head
             "work/%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
            :unnarrowed t)
           ("h" "home" plain "%?"
            :target
            (file+head
             "home/%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
            :unnarrowed t))))
...
(use-package
 eww
 :defer t
 :config (define-key eww-mode-map "w" 'scroll-down-command)
 (use-package
  shr
  :defer t
  :config
    ...
    (cl-reduce
     ...
     (cons
      (propertize (funcall #'identity
                           (buffer-name (car poe--popup-buffer-list)))
                  'face 'poe-echo-area)
      (cl-mapcar
       (lambda (buf)
         (concat
...
(use-package so-long :defer 1.3 :config (global-so-long-mode))
...
(use-package posframe :commands (posframe-hide posframe-show))
...
(use-package
 ...
 :init (add-hook 'xref-backend-functions #'ggtags--xref-backend 98)
 ...
 (defun string-inflection-camelcase-function (str)
   ...
   (setq str
         (split-string (string-inflection-underscore-function str)
                       "_"))
   (concat (downcase (car str)) (mapconcat 'capitalize (cdr str) "")))
