...
;; Maintain for personal use: lynnux
...
(defvar popup-kill-ring-pre-is-yank nil
  "*Indicate if pre-command is yank")

(defvar popup-kill-ring-final-selected nil
  "*The final selected item in popup-menu, use this for move selected string to top in `kill-ring'")

(defvar popup-kill-ring-kill-ring nil
  "*A copy of kill-ring when popup-kill-ring invoked. When previous command is `yank', the head item will be ignored.")
...
(defvar popup-kill-ring-keymap
  (let ((keymap (make-sparse-keymap)))
    ...
    (define-key keymap "\M-y" 'popup-kill-ring-next)
...
and `pos-tip.el'"
  ...
  (cond
   ...
   (t (let* ((index 0)
        ...
        ; copy kring, so other functions can use
        (setq popup-kill-ring-kill-ring kill-ring)
        (setq popup-kill-ring-pre-is-yank (eq last-command 'yank))
        
        ;; check wheher previous command is yank, most code from `yank-pop'
        (when popup-kill-ring-pre-is-yank
        ...
        (when popup-kill-ring-interactive-insert
          ...
          (popup-kill-ring-insert-item (if popup-kill-ring-pre-is-yank
                                           1
                                         0)))
        ...
        (unwind-protect
            (progn (setq item (popup-menu* kring
                   ...
                   (when item ; not nil only when popup's isearch return
                     (setq num (popup-kill-ring-get-index item))
                     (when num
                       ; find ture selected
                       (setq popup-kill-ring-final-selected num)
                       (insert (nth num popup-kill-ring-kill-ring))
                       )))
          (pos-tip-hide)
          ;; If `C-g' was typed, clear the inserted string. (7 is `C-g'.)
          (if (and popup-kill-ring-interactive-insert
                     (numberp last-input-event)
                     (= last-input-event ?\C-g))
              (popup-kill-ring-clear-inserted)
            (when popup-kill-ring-final-selected
              (popup-kill-ring-move-kill-ring popup-kill-ring-final-selected)
              (setq popup-kill-ring-final-selected nil))))))))

(defadvice popup-menu-event-loop (before pre-is-yank activate)
  (when popup-kill-ring-pre-is-yank
    (popup-next (with-no-warnings menu)); menu属于闭包是概念
))

(defun popup-kill-ring-move-kill-ring(num)
  (let* ((kill-ring-item (nth num popup-kill-ring-kill-ring)))
...
(defun popup-kill-ring-select ()
  ...
  (let* ((m (with-no-warnings menu))
    ...
    (setq popup-kill-ring-final-selected num) ;无论如何有结果是都会到这里
    (when popup-kill-ring-interactive-insert
      ...
      (when (and p (listp p))
        ...
        (unless (string= (buffer-substring (car p) (cdr p)) item)
          ...
          (return item)))
      (return)) ;这里还不能返回因为有内容了，这里返回nil，于是popup-kill-ring等待
                                        ;popup-menu*也就没有结果了
    (return item)))
         ...
         (progn
           ...
           (when num
             (funcall popup-kill-ring-kill-ring-show-func
                      (format "%s" (nth num popup-kill-ring-kill-ring))
...
(defun popup-kill-ring-insert-item (num)
  (let* ((item (format "%s" (nth num popup-kill-ring-kill-ring)))
           (s (point))
           (e (+ s (length item)))
           ol)
