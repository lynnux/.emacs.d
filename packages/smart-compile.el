...
;; Copyright (C) 2013 by lynnux, just for my personal use
...
(defvar smart-compile-lastest-command 
  nil)
(make-variable-buffer-local 'smart-compile-lastest-command)
...
(defun smart-compile-remember-latest-command ()
  (setq smart-compile-lastest-command compile-command))

(defadvice compile (after remember-command activate)
  (smart-compile-remember-latest-command))

(defadvice recompile (after remember-command activate)
  (smart-compile-remember-latest-command))

(defun smart-compile-c-compile ()
  (interactive)
  (smart-compile-no-side-effect "-c "))

(defun smart-compile-no-side-effect (&optional args)
  (interactive)
  (let( (name (buffer-file-name))
	(alist smart-compile-alist) 
	(case-fold-search nil)
	(function nil) 
	(lasted-command smart-compile-lastest-command))
    (while (and alist )
      (if (or
	  ...
	  (progn
	    ...
	    (if (stringp function)
		(progn
		  (let ((smart-compile-option-string args)) ; temperal use
		    (set (make-local-variable 'compile-command)
			       (smart-compile-string function)))
		  (call-interactively 'recompile) ; silent
		  (set (make-local-variable 'compile-command) ; no side effect
		       lasted-command)
		  (setq smart-compile-lastest-command lasted-command))
	      (if (listp function)
		  (eval function)
		))
	    (setq alist nil)
...
which is defined in `smart-compile-alist'."
  ...
  (save-excursion ; for C-g by lynnux
    (let ((name (buffer-file-name))
      ...
      (and (= arg 4) (setq smart-compile-check-makefile t)) ; reset var for make
